Bridge = function (options) {
    ...
    var archHeight = options.archHeight || 1.8;
    var archWidth = options.archWidth || 0.3;
    var postWidth = options.postWidth || archWidth+0.1;
    var postLength = options.postLength || 0.2;
    ...
    var archParts = options.archParts || 100;

    var overlap = options.overlap || 0.1;
    if(options.overlap==0)overlap=0;

    var template = [];

    var archAngle = Math.PI/archParts;

    this.bridgeObject = new Obj();
...
//---actual brigde-----------------------------------------------------------------------------------------------------
    ...
    for(var ib=0; ib<bridgesAmount; ib++){
        ...
        for(var i=0; i<template.length; i++){
            p = template[i];
            p.y += (p.x+archLength/2)*heightFactor;
        }
        ...
        //bottom
        var j,sp=1;
        var crossV, normal = new Vector(), lastNormal = new Vector(), tempV;

        if(!shareArchPoints)sp=2;
        for(var i=0; i<template.length-1/2*sp; i++){
            crossV = template[i].getDifferenceVector(template[(i+1)%template.length]);
            normal.x = crossV.y; normal.y = crossV.x;
            normal.normalize();
            tempV = normal.clone();
            if(i==template.length-1)normal = lastNormal;
            if(sp==1){
                normal.addVector(lastNormal);
                normal.normalize();
            }
            lastNormal = tempV;
            for(var k=0;k<sp;k++){
