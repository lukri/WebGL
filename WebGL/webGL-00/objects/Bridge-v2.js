Bridge = function (options) {
    ...
    var path = options.path || [new Vector(-5,0,-5), new Vector(0,0,0), new Vector(5,0,5)];
   ...
   // path.push(new Vector(0,0,2));
    ...
    for(var i=0; i<path.length; i++){
        ...
        if((((i==0)||(i==path.length-1))&&(!closePath))||(pA==180)||(pA==90)){
            ...
            pi2 = new Vector(bb,0,ba-c)
            pi3 = new Vector(0,0,c);
            pi4 = new Vector(-pi2.x,0,pi2.z);
            h=0;
            for(var k=-1;k<=1;k+=2){
            ...
            if(realVector.x>pi.x)pi.rotate *= -1;
            console.log(pi.rotate/Math.PI*180);



            //if(pi.z<pl.z)pi.rotate *= -1;
