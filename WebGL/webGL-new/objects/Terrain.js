...
Terrain = function (options) {
    ...
    var plantTrees = (options.plantTrees === true);
    ...
    */
    ...
    console.log("\t Find Train Path");

    var bridgeSegmentLength = 5;

    var testX,testZ;
    var normal;
    var trainPath = [];
    var bridgeV = new Vector(1,0,0);
    bridgeV.setLength(bridgeSegmentLength);
    var p1 = grid[maxGZ][maxGX].clone({y:2.5});
    var found = false, collide=false;
    ...
    lastTi = -1;
    while (ti<10){
        if(lastTi==ti){check++;}else{check=0;}
        lastTi = ti;
        if(check==50){break;}
        
        console.log(ti);
        var pNext = trainPath[ti].clone();
        pNext.addVector(bridgeV);
        testX = Math.round(pNext.x/(sideLength*repZ)*nZ+nZ/2);
        testZ = Math.round(pNext.z/(sideLength*repZ)*nZ+nZ/2)
        found = false;
        if(grid[testZ]){
            if(grid[testZ][testX]){
                normal = grid[testZ][testX].getNormal();
                normal.invert();
                normal.y = 0;
                angle=0;
                if(normal.getLength()!=0)
                    angle = bridgeV.getAngelBetween(normal)/Math.PI*180;
                if((angle<75)||(angle>105)||true){
                    collide=false;
                    if(collisionCount<72){
                       for(var i=ti-1;i>0;i--){
                           var testV = trainPath[i].getDifferenceVector(pNext);
                           if(testV.getLength()<bridgeSegmentLength){
                               collide=true;
                               collisionCount++;
                               break;
                           }
                    ...
                    collide=false;
                    if(!collide){
                        //pNext.y = grid[testZ][testX].y+1;
                        trainPath.push(pNext);
                        ti++;
                        collisionCount=0;
                        grid[testZ][testX].color = {r:10,g:0,b:0,a:1}
                        //found=true;
                    }
                }
            }
        ...
        if(!found){
            bridgeV.rotateAroundY(Math.PI/180*5);
            bridgeV.setLength(bridgeSegmentLength);
        }
    }

    grid[maxGZ][maxGX].color = {r:0,g:0,b:0,a:1};
    console.log(grid[maxGZ][maxGX].color);
    console.log(maxGZ+"..."+maxGX);

    console.log();

    this.getTrainPath = function(){return trainPath;};
    ...
    //random startpoint or highest point

    //add vector of sertain length and try out angle


    //take invertet normalvector with y=0 as gradient
    ...
    if(plantTrees){
        var total = nZ*nX;
        var reference = new Vector(0,1,0);
        for(var z=0;z<nZ;z++){
            for(var x=0;x<nX;x++){
                var v = grid[z][x];
                var normal = v.getNormal();
                var angle = reference.getAngelBetween(normal)/Math.PI*180;
                if((angle<=plantAngle)&&(v.y > 0.2)&&(v.y < (peekLevel-seaLevel))){
                    console.log("trees");
                    var terrainTree = new Tree();
                    terrainTree = terrainTree.getObject();
                    terrainTree.scale({xyz:0.05});
                    terrainTree.translate({x:v.x,y:v.y,z:v.z});
                    this.trees.addChild(terrainTree);
                    //v.color.r = 10;
                    treeNumber++;
                }
                if(treeNumber>=10)break;
            }
            if(treeNumber>=10)break;
        }
    }else{
        console.log("\t\t is turned off");
    }
    ...
    for(var pZ=0;pZ<nZ;pZ+=maxN-1){
        for(var pX=0;pX<nX;pX+=maxN-1){
            ...
            var terrainPart = new Obj("preloaded",{
